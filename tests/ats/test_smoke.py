"""ATS smoke for the vm-manager chart.
...
Deployment is ready with the image the branch build pushed. The guest image
fetch is off on this cluster (a branch publishes no artifact) and whether the
node has KVM is not asked; readiness depends on neither.
"""
...
DEPLOYMENT = "vm-manager"
# `helm --wait` already returned, so the Deployment is normally ready when the
# tests start; the budget only absorbs a slow first image pull (the runtime
# image carries QEMU, swtpm and OVMF).
TIMEOUT = 300
