...
| guestImage.resources | object | `{"requests":{"cpu":"100m","memory":"128Mi"}}` | Resources of the init container. It streams the artifact's files to the volume, so its memory is the page cache of what it writes, and a memory limit turns a registry that delivers faster than the volume writes back into an OOM kill of the pull (seen at 256Mi with a registry on the node). The chart sets requests only; a limit is the installation's call. |
