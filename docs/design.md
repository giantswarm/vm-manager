...
| T2 image | mkosi build + `image-verify`: `systemd-dissect`, `ukify inspect`, `systemd-measure calculate` | minutes, cached | every PR touching images/, vm-agent or the workflow; push to main; nightly through e2e | `image.yml`, `archlinux:latest` container (`--privileged`) on `ubuntu-26.04`, mkosi caches keyed weekly; artifact `guest-image` |
